...
summa_size = 0
summa_size += sys.getsizeof(matrix)
summa_size += sys.getsizeof(min_list)
summa_size += sys.getsizeof(i)
summa_size += sys.getsizeof(string)
summa_size += sys.getsizeof(j)

print('Переменные занимают', summa_size)
...
summa = 0
for size in r:
    summa += sys.getsizeof(size)
print('Размер всех элементов в листе', summa)
